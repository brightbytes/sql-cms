shared_examples 'yaml helper methods' do
  ...
  # These next two tests are extraordinarily silly (i.e. quite pathetic) - they should really be separate specs in the parent files.  Bah.
  ...
  it "should provide instance- and class-level interpolation methods that handle arrays" do
    ...
    expect(subject.class.interpolate(string: sql, params: subject.params)).to eq("SELECT bar FROM ('check','this','out')")
