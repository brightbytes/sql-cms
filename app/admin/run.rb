ActiveAdmin.register Run do
  ...
  show do
    ...
    panel 'Run Step Logs' do
      ...
      table_for(resource.run_step_logs.order('id'), sortable: true) do
        ...
        column(:json_output) { |log| code(truncate(pretty_print_as_json(log.step_output), length: MAX_STEP_OUTPUT_SIZE, omission: ' ...')) }
      end
    end
  ...
  member_action :dump_execution_plan, method: :put do
    send_data resource.execution_plan_dump, filename: "#{resource.schema_name}.json"
  end
