...
module RunnerFactory
      ...
      open(url) do |stream|
        begin
          ...
          migration_s = <<-SQL.strip_heredoc
            ...
            #{indexes_s}
          SQL
          run.eval_in_schema(migration_s, in_postgres: !run.use_redshift?)

          header_s = header_a.join(', ')
          sql = "COPY #{table_name} (#{header_s}) FROM STDIN WITH CSV"

          run.copy_from_in_schema(sql: sql, enumerable: stream, in_postgres: !run.use_redshift?)
  ...
  module RailsMigrationRunner
    ...
    def run(run:, plan_h:)
      ...
      run.eval_in_schema(plan_h[:sql], in_postgres: !run.use_redshift?)
    end
  end
  ...
  module CopyFromRunner
    ...
    def run(run:, plan_h:)
      ...
      open(url) { |stream| run.copy_from_in_schema(sql: plan_h[:interpolated_sql], enumerable: stream, in_postgres: !run.use_redshift?) }
    end
  end
  ...
  module SqlRunner
    ...
    def run(run:, plan_h:)
      run.execute_in_schema(plan_h[:interpolated_sql], in_postgres: !run.use_redshift?)
    end
  end
  ...
  module CopyToRunner
    ...
    def run(run:, plan_h:)
      ...
      Tempfile.open(s3_file.s3_file_name, Dir.tmpdir, mode: IO::RDWR) do |stream|
        run.copy_to_in_schema(sql: plan_h[:interpolated_sql], writeable_io: stream, in_postgres: !run.use_redshift?).tap do
          stream.rewind
  ...
  module ValidationRunner
    ...
    def run(run:, transform_validation_h:)
      ...
      if ids = run.select_values_in_schema(transform_validation_sql, in_postgres: !run.use_redshift?).presence
        {
  ...
  module WorkflowDataQualityReportRunner
    ...
    def run(run:, plan_h:)
      run.select_all_in_schema(plan_h[:interpolated_sql], in_postgres: !run.use_redshift?)&.to_hash
    end
  end
